    ...
    optimizerBuilder
      .withLeaseRejectAction(new Action1[VirtualMachineLease]() {
        def call(lease: VirtualMachineLease) {
         ...
         println("70 in org/apache/flink/mesos/scheduler/LaunchCoordinator.scala ")
  ...
  when(Idle) {
    ...
    case Event(offers: ResourceOffers, data: GatherData) =>
      ...
      for(offer <- offers.offers().asScala) {
        ...
        println("115 org/apache/flink/mesos/scheduler/LaunchCoordinator.scala declineOffer")
      }
      stay()
  ...
  when(GatheringOffers, stateTimeout = GATHER_DURATION) {
    ...
    case Event(offers: ResourceOffers, data: GatherData) =>
      ...
      if(LOG.isInfoEnabled) {
        ...
        LOG.info(s"156 Received offer(s) of $mem MB, $cpus cpus, $gpus gpus:")
        for(l <- leases) {
          val reservations = l.getResources.asScala.map(_.getRole).toSet
          LOG.info(
            s" 160 ${l.getId} from ${l.hostname()} of ${l.memoryMB()} MB," +
